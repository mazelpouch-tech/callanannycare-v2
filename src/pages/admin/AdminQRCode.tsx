...
import { Download, Printer, Copy, Check, QrCode, ExternalLink, Handshake } from "lucide-react";
...
export default function AdminQRCode() {
  ...
  const cards: QRCard[] = [
    {
      id: "qr-booking",
      title: "Book a Nanny",
      description: "Parents scan this to go directly to the booking page",
      url: `${origin}/book`,
      filename: "qr-book-nanny.png",
      icon: "qr",
    },
    ...PARTNER_LIST.map((p) => ({
      id: `qr-partner-${p.slug}`,
      title: `${p.name} Booking`,
      description: `Exclusive booking portal for ${p.name} guests`,
      url: `${origin}/book/${p.slug}`,
      filename: `qr-${p.slug}-booking.png`,
      icon: "partner" as const,
    })),
    {
      id: "qr-parent-form",
      title: "Parent Form",
      description: "Parents scan this to fill the registration / child info form",
      url: `${origin}/parent-form`,
      filename: "qr-parent-form.png",
      icon: "form",
    },
  ];
  ...
  const renderIcon = (icon: QRCard["icon"]) => {
    if (icon === "partner") {
      return (
    ...
    <div className="space-y-8">
      ...
      <div className="grid md:grid-cols-2 gap-6" ref={qrRef}>
        {cards.map((card) => (
          <div
            key={card.id}
            className="bg-card rounded-2xl border border-border p-8 flex flex-col items-center text-center"
          >
            {renderIcon(card.icon)}
            <h2 className="text-lg font-serif font-bold text-foreground mb-1">{card.title}</h2>
            ...
            <div className="flex gap-2 w-full mt-auto">
              <button
                onClick={() => handleDownload(card.id, card.filename)}
              ...
              <button
                onClick={() => handleCopy(card.id, card.url)}
                className="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2 bg-muted text-foreground rounded-xl text-sm font-medium hover:bg-muted/80 transition-colors"
              >
                {copiedId === card.id ? (
