...
export default function AdminMessages() {
  ...
  // Build channel list: group_all + dm_<nannyId> for each active nanny
  const activeNannies = nannies.filter((n) => n.status === "active");
  const channels = [
    { channel: "group_all", name: "Group Chat", subtitle: "All nannies" },
    ...activeNannies.map((n) => ({
      channel: `dm_${n.id}`,
      name: n.name,
              ...
              <div className="flex items-center gap-3 px-4 py-3 border-b border-border bg-card">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    {activeChannel === "group_all" ? "All nannies" : "Direct message"}
                  </p>
                </div>
              </div>
