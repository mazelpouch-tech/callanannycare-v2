...
import { Calculator, Copy, Check, Moon, Sun, Plus, Minus } from "lucide-react";
...
export default function AdminQuotes() {
  ...
  const baseCost = RATE * hoursPerDay * days;
  ...
  const quoteText = [
    ...
    ...(hotel ? [`Hotel: ${hotel}`] : []),
    ``,
    `${days} day${days > 1 ? "s" : ""} x ${hoursPerDay} hour${hoursPerDay !== 1 ? "s" : ""}/day = ${days * hoursPerDay} total hours`,
    `Rate: ${RATE}€/hour`,
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="bg-gradient-to-br from-primary/5 to-primary/10 border border-primary/20 rounded-xl p-5 space-y-3">
        ...
        <button
          onClick={handleCopy}
          className="w-full mt-2 flex items-center justify-center gap-2 bg-primary text-white py-2.5 rounded-lg hover:bg-primary/90 transition-colors font-medium"
        >
          {copied ? (
            <>
            ...
            <>
              <Copy className="h-4 w-4" /> Copy Quote to Clipboard
            </>
          )}
        </button>

        <p className="text-xs text-gray-500 text-center">
          Copy and paste into WhatsApp or email to share with the parent
        </p>
      </div>
    </div>
  );
