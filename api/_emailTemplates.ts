...
const strings = {
  en: {
    ...
    paymentCollectedByText: 'Please note that payment for your childcare service will be collected by a member of our staff by the end of your stay — not by your hotel or accommodation.',
    invoice: 'INVOICE',
  ...
  fr: {
    ...
    paymentCollectedByText: 'Veuillez noter que le paiement de votre service de garde sera collecté par un membre de notre équipe avant la fin de votre séjour — et non par votre hôtel ou hébergement.',
    invoice: 'FACTURE',
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
      <tr>
        ...
        <td style="vertical-align:top;width:50%;padding-left:16px;">
          ...
          ${data.billedTo ? `<p style="margin:2px 0 0;color:#666;font-size:13px;font-style:italic;">On behalf of ${data.clientName}</p>` : ''}
...
// ============================================================
// Admin Weekly Payroll Summary Email
// ============================================================

export interface WeeklySummaryEmailData {
  adminEmails: string[];
  periodLabel: string;
  nannySummaries: Array<{
    nannyName: string;
    bookings: Array<{
      id: number;
      date: string;
...
export async function sendWeeklySummaryEmail(data: WeeklySummaryEmailData): Promise<boolean> {
  ...
  // Build per-nanny sections
  const nannySections = data.nannySummaries.map(n => {
    const rows = n.bookings.map(b =>
      `<tr>
        <td style="padding:6px 10px;border-bottom:1px solid #f0f0f0;font-size:12px;color:#1a1a1a;">#${b.id}</td>
        <td style="padding:6px 10px;border-bottom:1px solid #f0f0f0;font-size:12px;color:#1a1a1a;">${b.date}</td>
        <td style="padding:6px 10px;border-bottom:1px solid #f0f0f0;font-size:12px;color:#666;">${b.clientName}</td>
        <td style="padding:6px 10px;border-bottom:1px solid #f0f0f0;font-size:12px;color:#666;">${b.startTime} - ${b.endTime}</td>
        <td style="padding:6px 10px;border-bottom:1px solid #f0f0f0;font-size:12px;color:#1a1a1a;text-align:right;">${b.hours.toFixed(1)}h</td>
        <td style="padding:6px 10px;border-bottom:1px solid #f0f0f0;font-size:12px;color:#1a1a1a;font-weight:600;text-align:right;">${b.pay} DH</td>
      </tr>`
    ).join('');

    return `
      <h3 style="margin:20px 0 8px;color:#1a1a1a;font-size:15px;font-weight:600;">${n.nannyName} — ${n.totalHours.toFixed(1)}h / ${n.totalPay.toLocaleString()} DH</h3>
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #f0f0f0;border-radius:8px;overflow:hidden;margin-bottom:16px;">
        <tr style="background:#fafafa;">
          <th style="padding:6px 10px;text-align:left;font-size:10px;color:#666;text-transform:uppercase;">Ref</th>
          <th style="padding:6px 10px;text-align:left;font-size:10px;color:#666;text-transform:uppercase;">Date</th>
          <th style="padding:6px 10px;text-align:left;font-size:10px;color:#666;text-transform:uppercase;">Client</th>
          <th style="padding:6px 10px;text-align:left;font-size:10px;color:#666;text-transform:uppercase;">Time</th>
          <th style="padding:6px 10px;text-align:right;font-size:10px;color:#666;text-transform:uppercase;">Hours</th>
          <th style="padding:6px 10px;text-align:right;font-size:10px;color:#666;text-transform:uppercase;">Pay</th>
        </tr>
        ${rows}
      </table>
    `;
  ...
  const content = `
    <div style="text-align:center;margin-bottom:24px;">
      <h2 style="margin:0 0 6px;color:#1a1a1a;font-size:22px;font-family:Georgia,'Times New Roman',serif;">Weekly Payroll Summary</h2>
      <p style="margin:0;color:#666;font-size:15px;">Here's the payroll recap for the completed pay period.</p>
    </div>

    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #f0f0f0;border-radius:8px;overflow:hidden;margin-bottom:24px;">
      ${row('Period', data.periodLabel)}
      ${row('Nannies', `${data.nannySummaries.length}`)}
      ${row('Total Shifts', `${data.nannySummaries.reduce((s, n) => s + n.bookings.length, 0)}`)}
      ${row('Total Hours', `${data.grandTotalHours.toFixed(1)}h`)}
      ${row('Total Payroll', `${data.grandTotalPay.toLocaleString()} DH`)}
    </table>

    ${data.nannySummaries.length > 0 ? nannySections : '<p style="color:#666;font-size:14px;text-align:center;margin:16px 0;">No completed shifts this period.</p>'}

    <div style="text-align:center;margin:32px 0;">
      <a href="${baseUrl}/admin/nannies" style="display:inline-block;background:linear-gradient(135deg,#ef4444,#f97316);color:#ffffff;font-size:16px;font-weight:600;text-decoration:none;padding:14px 32px;border-radius:50px;box-shadow:0 4px 14px rgba(239,68,68,0.3);">View Admin Dashboard</a>
    </div>
  ...
  try {
    await resend.emails.send({
      from: fromAddress,
      to: data.adminEmails,
      subject: `Weekly Payroll Summary — ${data.periodLabel}`,
      html: emailWrapper(content),
    });
    return true;
  } catch (err) {
